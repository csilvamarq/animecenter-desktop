      ...
      {episode.length > 1 ? (
        ...
        >
          <div style={{ display: "flex", gap: "2%", alignItems: "center" }}>
            <AiFillLeftCircle
              color={currentEpisode < 2 ? "grey" : "black"}
              onClick={() => {
                if (currentEpisode > 1) {
            ...
            />
            <h1>
              {state.name} {currentEpisode}
            </h1>
            <AiFillRightCircle
              size={30}
              color={currentEpisode < state.total ? "black" : "grey"}
              onClick={() => {
                if (currentEpisode < state.total) {
                  setCurrentEpisode(currentEpisode + 1);
                  setEpisode("");
                }
              }}
            />{" "}
            <Button onClick={() => setDowload(true)}>Descargar</Button>
          </div>
