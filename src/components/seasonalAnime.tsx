...
const SeasonalAnime: React.FC = () => {
    ...
    useQuery({
        queryKey: ['lastAnime'], queryFn: async () => {
            return axios.get("http://localhost:3002/lastAnime").then(({ data }) => setSeasonalAnime(data))
        }
    })
    return (
        <>
                ...
                <div style={{ padding: 24, height: "100%", textAlign: "center", color: "black", background: colorBgContainer, overflowY: "auto" }}>
                    <Row>
                        {seasonalAnime?.map((item: SeasonalAnimeType, index: number) => {
                            return (<Col style={{ padding: "1%", paddingTop: "8%" }} key={index} span={6}>
                                <h2 style={{ fontFamily: "Roboto" }}>{<ShowMoreText lines={3}
                                ...
                                <p style={{ fontFamily: "Roboto" }}>Episodio {item.episode}</p>
                            </Col>)
                        })}
                    </Row>
                </div>
